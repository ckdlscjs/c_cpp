	...
	float screenDepth, float screenNear)
{
    ...
    blendStateDescription.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE; //소스컬러의 블렌딩 방법을 나타낸다
    blendStateDescription.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA; //대상컬러의 블렌딩 방법을 나타낸다
    blendStateDescription.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;	//블렌딩 연산 방법을 나타낸다
    blendStateDescription.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE; 
    blendStateDescription.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
    blendStateDescription.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    blendStateDescription.RenderTarget[0].RenderTargetWriteMask = 0x0f; //렌더타겟의 컬러채널중 어떤것을 사용할것인가?
	/*이 멤버는 4비트 마스크로 구성되어 있으며, 0x01은 빨간색, 0x02는 녹색, 0x04는 파란색, 0x08은 알파 채널을 나타냅니다.
	위 코드에서는 0x0f로 설정되어 있으므로, 모든 색상 채널을 사용하도록 설정되었습니다.*/
...
void D3DClass::TurnOnAlphaBlending()
{
	...
	/*
	마지막 인수는 Sample Mask 값으로, 블렌딩을 적용할 Sample Mask를 나타냅니다. 
	이 값은 32비트 정수 값으로, 각 비트는 하나의 샘플을 나타내며, 0으로 설정된 비트는 해당 샘플이 블렌딩에 사용되지 않음을 나타냅니다. 
	0xffffffff로 설정하면 모든 샘플이 블렌딩에 사용됩니다. 따라서 일반적으로는 이 값을 0xffffffff로 설정하는 것이 일반적입니다.
	*/
}
