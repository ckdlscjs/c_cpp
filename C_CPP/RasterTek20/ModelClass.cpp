...
void ModelClass::CalculateTangentBinormal(TempVertexType vertex0, TempVertexType vertex1, TempVertexType vertex2,
										  VectorType& tangent, VectorType& binormal)
{
	...
	*/
	float vector_e0[3], vector_e1[3], vector_deltaU[2], vector_deltaV[2];

	// 현재 표면의 두 벡터를 계산합니다.
	vector_e0[0] = vertex1.x - vertex0.x;
	vector_e0[1] = vertex1.y - vertex0.y;
	vector_e0[2] = vertex1.z - vertex0.z;

	vector_e1[0] = vertex2.x - vertex0.x;
	vector_e1[1] = vertex2.y - vertex0.y;
	vector_e1[2] = vertex2.z - vertex0.z;

	// tu 및 tv 텍스처 공간 벡터를 계산합니다.
	vector_deltaU[0] = vertex1.tu - vertex0.tu;		//a
	vector_deltaV[0] = vertex1.tv - vertex0.tv;		//b
									  
	vector_deltaU[1] = vertex2.tu - vertex0.tu;		//c
	vector_deltaV[1] = vertex2.tv - vertex0.tv;		//d

	// 탄젠트 / 바이 노멀 방정식의 분모를 계산합니다. -> 1 / ad-bc
	float den = 1.0f / (vector_deltaU[0] * vector_deltaV[1] -  vector_deltaV[0] * vector_deltaU[1]);

	// 교차 곱을 계산하고 계수로 곱하여 접선과 비 구식을 얻습니다.
	//denom * adjMat_Row * MatE_Col
	tangent.x = (vector_deltaV[1] * vector_e0[0] - vector_deltaV[0] * vector_e1[0]) * den;
	tangent.y = (vector_deltaV[1] * vector_e0[1] - vector_deltaV[0] * vector_e1[1]) * den;
	tangent.z = (vector_deltaV[1] * vector_e0[2] - vector_deltaV[0] * vector_e1[2]) * den;

	binormal.x = (-vector_deltaU[1] * vector_e0[0] + vector_deltaU[0] * vector_e1[0]) * den;
	binormal.y = (-vector_deltaU[1] * vector_e0[1] + vector_deltaU[0] * vector_e1[1]) * den;
	binormal.z = (-vector_deltaU[1] * vector_e0[2] + vector_deltaU[0] * vector_e1[2]) * den;
