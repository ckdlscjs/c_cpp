...
//-----------------------------------------------------------------------------
//Ray와 Sphere의 교차판정, t를 반환한다(교차점까지의 스칼라)
BOOL IntersectRaySphere( FXMVECTOR Origin, FXMVECTOR Direction, const Sphere* pVolume, FLOAT* pDist )
{
    ...
    //방향벡터에 투영하여 중심에 수선의발을 내리는 벡터를 만든다
    ...
    //시작점이 구의바깥에 존재하면서 방향또한 구의반대방향, 즉 만날수없음
    ...
    //구 안에 있는가 없는가를 체크하는 boolean mask 벡터
    XMVECTOR OriginInside = XMVectorLessOrEqual( l2, r2 );
    //결과에 따라 t1, t2의 가까운점을 채택
