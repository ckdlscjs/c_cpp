...
public:
	...
	inline Vector2(const DirectX::XMFLOAT2& v) : m_vec(DirectX::XMLoadFloat2(&v)) {} // XMLoadFloat2는 z,w를 0으로 설정
	...
	inline float Dot(const Vector2& other) const { return DirectX::XMVectorGetX(DirectX::XMVector2Dot(m_vec, other.m_vec)); }
	...
	friend inline float Cross(const Vector2& v1, const Vector2& v2); // 2D Cross는 스칼라를 반환합니다.
...
// 닷 프로덕트 (내적)
inline float Dot(const Vector2& v1, const Vector2& v2) { return DirectX::XMVectorGetX(DirectX::XMVector2Dot(v1.m_vec, v2.m_vec)); }

// 2D 외적 (스칼라 값 반환)
inline float Cross(const Vector2& v1, const Vector2& v2) { return DirectX::XMVectorGetZ(DirectX::XMVector2Cross(v1.m_vec, v2.m_vec)); }
