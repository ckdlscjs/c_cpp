...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"../RasterTek06/lightvs.hlsl", L"../RasterTek06/lightps.hlsl");
}
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	/*
	matrixBufferDesc.CPUAccessFlags = 0으로 선언하면 UpdateSubresource만 가능하고 Map,Unmap은 사용이불가능하다(cpu쪽에서 상수버퍼에접근하는게불가능하므로)
	D3D11_CPU_ACCESS_WRITE로 선언시 map,unmap을 사용이가능하다
	*/
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	...
	XMMATRIX worldInvTrans = XMMatrixTranspose(XMMatrixInverse(&XMMatrixDeterminant(worldMatrix), worldMatrix));
	...
	dataPtr->worldInvTrans = worldInvTrans;
	// 상수 버퍼의 잠금을 풉니다.
