...
bool BitmapClass::InitializeBuffers(ID3D11Device* device)
{
	...
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT; //D3D11_USAGE_DYNAMIC;
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// 기존에 할당되어있는 m_pVertexBuffer가 가지는 데이터의 주소를 mappedResource의 pData로부터 가져온다(VetexType*)mappedResource.pData 로 지정
	...
	// 데이터를 정점 버퍼에 복사합니다.
	// m_pVertexBuffer가 가리키는 주소인 verticesPtr에 새로 지정된 데이터인 vertices의 데이터를 옮긴다
	memcpy(verticesPtr, (void*)vertices, (sizeof(VertexType) * m_vertexCount));

	// 정점 버퍼의 잠금을 해제합니다.
	// 해당부분의 역할이 UpdateSubresource와 비슷하다, 차이점은 Unmap이후에 다시 메모리에대한 데이터에 접근하려면 다시 Map을 해줘야하지만 UpdateSubresource는 그럴필요가없다
	...
	vertices[5].texture = XMFLOAT2(1.0f, 1.0f);
	*/
	deviceContext->UpdateSubresource(m_vertexBuffer, NULL, NULL, vertices, NULL, NULL);
