...
void GraphicEngine::Frame()
{
	...
	cc0.matView = GetMat_ViewMatrix(XMFLOAT3(0.0f, 300.0f, -500.0f), XMFLOAT3(0.0f, 0.0f, 0.0f));
	//cc0.matProj = GetMat_Ortho(m_fWidth, m_fHeight, -4000.0f, 4000.0f);
	cc0.matProj = GetMat_Perspective(m_fWidth, m_fHeight, 90.0f, 0.1f, 4000.0f);
...
VertexBuffer* GraphicEngine::CreateVertexBuffer(void* vertices, UINT size_vertex, UINT size_vertices)
{
...
InputLayout* GraphicEngine::CreateInputLayout(D3D11_INPUT_ELEMENT_DESC* pInputElementDescs, UINT size_layout)
{
...
IndexBuffer* GraphicEngine::CreateIndexBuffer(void* indices, UINT size_indices)
{
...
VertexShader* GraphicEngine::CreateVertexShader(std::wstring shaderName, std::string entryName, std::string target)
{
	if(m_pBlob_VS)
		m_pBlob_VS->Release();
...
PixelShader* GraphicEngine::CreatePixelShader(std::wstring shaderName, std::string entryName, std::string target)
{
	if(m_pBlob_PS)
		m_pBlob_PS->Release();
