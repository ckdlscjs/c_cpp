...
inline Vector3 GetAxesRightFromWorld(const Matrix4x4& matWorld)
{
	...
	return Vector3(matWorld[0].GetX(), matWorld[1].GetX(), matWorld[2].GetX()) / vScale.GetX();
}

inline Vector3 GetAxesUpFromWorld(const Matrix4x4& matWorld)
{
	Vector3 vScale = GetScaleFromWorld(matWorld);
	return Vector3(matWorld[0].GetY(), matWorld[1].GetY(), matWorld[2].GetY()).Normalize() / vScale.GetY();
}

inline Vector3 GetAxesForwardFromWorld(const Matrix4x4& matWorld)
{
	Vector3 vScale = GetScaleFromWorld(matWorld);
	return Vector3(matWorld[0].GetZ(), matWorld[1].GetZ(), matWorld[2].GetZ()).Normalize() / vScale.GetZ();
}
