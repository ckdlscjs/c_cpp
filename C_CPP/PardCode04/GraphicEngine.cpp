...
bool GraphicEngine::Init(Window* pWindow)
{
	...
	ID3D11DeviceContext* pD3D_DeviceContext;
	HRESULT hResult = 0;
	for (UINT driver_type_idx = 0; driver_type_idx < num_drivers; driver_type_idx++)
	{
		...
		(
			...
			&pD3D_DeviceContext					//_COM_Outptr_opt_ ID3D11DeviceContext * *ppImmediateContext, 반환할ID3DDeviceContext객체
		);
	...
	m_pCDeviceContext = new DeviceContext();
	m_pCDeviceContext->Init(pD3D_DeviceContext);
	return true;
}
...
bool GraphicEngine::Release()
{
	...
	delete m_pCDeviceContext;
	m_pCDeviceContext = nullptr;
