...
VS_OUTPUT vsmain(VS_INPUT input, uint instanceId : SV_InstanceID)
{
    ...
    output.pos0 = mul(float4(posAnim, 1.0f), batchMatrix[matrixID].matWorld);
   // output.pos0 = mul(input.pos0, matWorld);
    ...
    output.tangent0 = float4(mul(input.tangent0.xyz, (float3x3) batchMatrix[matrixID].matInvTrans), 0.0f);
    output.binormal0 = float4(mul(input.binormal0.xyz, (float3x3) batchMatrix[matrixID].matInvTrans), 0.0f);
