...
VS_OUTPUT vsmain(VS_INPUT input, uint instanceId : SV_InstanceID)
{
    ...
    //월드변환
    output.pos0 = mul(float4(input.pos0.xyz, 1.0f), batchMatrix[matrixID].matWorld);
    float3 worldNormal = normalize(mul(input.normal0.xyz, (float3x3) batchMatrix[matrixID].matInvTrans));
    //노말값의 확장크기를 더함
