...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"../RasterTek12/fontvs.hlsl", L"../RasterTek12/fontps.hlsl");
}
...
//사용해야할 쉐이더를 컴파일 및 VS/PS 로우자원 및 InputLayout객체또한 생성한다
bool FontShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	constantBufferDesc.Usage = D3D11_USAGE_DYNAMIC;						//동적자원, 즉 MAP/UNMAP을 통해 기하정보를 업데이트한다
	constantBufferDesc.ByteWidth = sizeof(ConstantBufferType);
	constantBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	constantBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;			//동적자원, 즉 MAP/UNMAP을 통해 기하정보를 업데이트한다
