	...
	XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../RasterTek12/data/fontdata.txt", L"../RasterTek12/data/font.dds");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, "Weather is very bad.", 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
//넘겨받은 sentenc객체에 최대길이(maxlength)를 지정하고 vertex,index 의 갯수를 이에따라 최대크기에 맞게끔 초기화하며 버퍼를 생성한다
//해당 크기는 최대크기이며 동적버퍼를 사용하므로 추후 MAP/UNMAP과정에서 다른크기로 바뀌어가며 사용된다
...
//앞서 초기화한 sentence객체에 입력받은 text문장과 시작위치, 컬러 를 기반으로 사용될 문장내용을 업데이트한다
	...
	ID3D11DeviceContext* deviceContext)
{
	...
	// 시작위치를 drawX, Y로 받고 이를 화면전체길이 width, height를 기반으로 (0, 0) 화면좌표 중심에서의 상대값으로 데카르트좌표계단위로 좌표값을 구현한다
